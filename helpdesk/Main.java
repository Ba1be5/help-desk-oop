...
import helpdesk.service.SmsNotificationService;
...
public class Main {
    public static void main(String[] args) {
        ...
        // 5. Переоткрываем заявку (Задание при защите №1)
        ticket.reopen();
        System.out.println("Status after reopen: " + ticket.getStatus());

        ticketService.closeTicket(ticket); // здесь будет ошибка, потому что статус REOPENED
        System.out.println("Status: " + ticket.getStatus());

        // 6. Сохраняем заявку
        repository.add(ticket);

        // 7. Выводим все заявки
        System.out.println("\nAll tickets:");
